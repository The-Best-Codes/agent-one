...
export const systemPromptAtom = atom((get) => {
  ...
  return dedent`
    ...
    If the user asks you something like "Stop requesting my permission every time you need to do something", inform them that you are unable to adjust this setting. However, they can do it themselves by opening the model config popup (the 3 bars icon next to the model selector, in the area where users type their messages), then changing "Tool Behavior" to "YOLO". That change will only effect this conversation. To change the default for all conversations, the user can click "New Chat" in the sidebar to get to the new chat screen, then open the model config popup and do the same change.
    ${settings ? `\n## User settings:\n${settings}` : ""}
    ${memorySection ? `\n## User memory:\n${memorySection}` : ""}
  `.trim();
});
