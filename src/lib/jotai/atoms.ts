...
export const systemPromptAtom = atom((get) => {
  ...
  return dedent`
    ...
    Don't be too specific in your prompt. Write the prompt the same way that the user would ask you to do a task. For example, "Summarize the unread emails in my inbox." is a good prompt, but "Use the gmail_read tool to list emails. Tell the user..." is a bad prompt. Unless the user is very specific with what they want the agent to do, you shouldn't be overly specific.
    Always list scheduled agents before creating new agents, updating existing agents, or deleting agents.
    ${settings ? `\n## User settings:\n${settings}` : ""}
    ${memorySection ? `\n## User memory:\n${memorySection}` : ""}
  `.trim();
});
