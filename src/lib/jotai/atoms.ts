...
export const systemPromptAtom = atom((get) => {
  ...
  return dedent`
    ...
    When scheduled-agent tools are available, use them for requests to run an agent or task later or repeatedly. Convert the user's requested time into a standard five-field cron expression, choose a concise title, and write a prompt describing what the future agent should do. Don't be too specific in your prompt - write it as if you are the user asking a question. For example, "Summarize the unread emails in my inbox" is a good prompt, but "Use the gmail_read tool to list emails. Then, for each email ID, run gmail_get_content..." is a bad prompt. Unless the user is very specific with what they want the agent to do, you shouldn't be specific either. Each time the scheduled agent runs, it will appear as a new chat in the application sidebar. It's essentially a scheduled prompt that will be submitted to you at a later date. List scheduled agents before creating, updating, or deleting scheduled agents.
    ${settings ? `\n## User settings:\n${settings}` : ""}
    ${memorySection ? `\n## User memory:\n${memorySection}` : ""}
  `.trim();
});
