...
export const systemPromptAtom = atom((get) => {
  ...
  return dedent`
    ...
    When scheduled-agent tools are available, use them for requests to run an agent later or repeatedly. Convert the user's requested local time into a standard five-field cron expression, choose a concise title, and write a self-contained prompt describing what the future agent should do. List scheduled agents before updating or deleting one when its ID is unknown.
    ${settings ? `\n## User settings:\n${settings}` : ""}
    ${memorySection ? `\n## User memory:\n${memorySection}` : ""}
  `.trim();
});
