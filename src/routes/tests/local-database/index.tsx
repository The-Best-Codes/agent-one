...
export default function LocalDatabaseTestRoute() {
  ...
  const runBenchmark = async () => {
    ...
    try {
      ...
      addLog(
        `Preparing ${
          BENCHMARK_CHAT_COUNT
        } temporary chats with ${MESSAGES_PER_CHAT} messages each.`,
      );

      await measure(
        "Write workload",
        `${BENCHMARK_CHAT_COUNT.toLocaleString()} chats, ${(
          BENCHMARK_CHAT_COUNT * MESSAGES_PER_CHAT
        ).toLocaleString()} long messages`,
          ...
          for (let iteration = 0; iteration < SEARCH_ITERATIONS; iteration += 1) {
            ...
            if ((iteration + 1) % 100 === 0 || iteration + 1 === SEARCH_ITERATIONS) {
              addLog(
                `Search progress: ${(
                  iteration + 1
                ).toLocaleString()} / ${SEARCH_ITERATIONS.toLocaleString()} queries`,
              );
            }
          ...
          <Card>
            ...
            <CardContent className="flex flex-col gap-4">
              <div className="text-muted-foreground text-sm">
                {`Workload: ${BENCHMARK_CHAT_COUNT.toLocaleString()} chats, ${
                  MESSAGES_PER_CHAT
                } long messages per chat, ${SEARCH_ITERATIONS.toLocaleString()} search passes.`}
              </div>
