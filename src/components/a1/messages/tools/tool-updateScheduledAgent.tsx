...
interface UpdateScheduledAgentInput {
  id?: string;
  title?: string;
  schedule?: string;
  prompt?: string;
...
export const MessagePartToolUpdateScheduledAgent = ({ part }: { part: ToolUIPart }) => {
  ...
  const input = (part.input ?? {}) as UpdateScheduledAgentInput;
  const output = part.output as UpdateScheduledAgentInput | undefined;
    ...
    case "input-available": {
      ...
      return (
        <div key={callId} className="flex items-center gap-1 text-sm font-bold">
          ...
          <span>{`Updating scheduled agent${shortLabel}`}</span>
        </div>
      );
    }
      ...
      return (
        ...
        >
          <AccordionItem
            value={callId}
            className={cn(
              "group/sched-update border-border w-fit max-w-full rounded-md border-0 transition-[padding] duration-200",
          ...
          >
            ...
            >
              <span className="max-w-2xl truncate">{`Updated scheduled agent${doneLabel}`}</span>
            </AccordionTrigger>
            <AccordionContent className="p-0 pt-2">
              <Detail input={merged} />
            </AccordionContent>
    ...
    case "output-error":
      ...
      return (
        <ToolErrorAccordion
          ...
          title="Error updating scheduled agent"
        />
      );
    default:
      return (
...
MessagePartToolUpdateScheduledAgent.displayName = "MessagePartToolUpdateScheduledAgent";
