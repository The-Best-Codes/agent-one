...
- Allow changing stop button behavior in settings: Stop at next stopping point (wait until status is streaming), abort immediately (show stop button even when status is submitted), or only show stop option when at a stopping point (current behavior)
